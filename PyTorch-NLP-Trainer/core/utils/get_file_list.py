...
def save_file_list(file_dir, prefix="", postfix=["*.json"], only_id=True, val_nums=200, shuffle=False, nums=None):
    """保存文件列表"""
    trainval = os.path.join(os.path.dirname(file_dir), "trainval.txt")
    train = os.path.join(os.path.dirname(file_dir), "train.txt")
    val = os.path.join(os.path.dirname(file_dir), "val.txt")
    ...
    file_utils.write_list_data(trainval, file_list)
    random.seed(100)
    random.shuffle(file_list)
    # val_nums = len(file_list)
    val_list = file_list[0:val_nums]
    train_list = file_list[val_nums:]
    file_utils.write_list_data(train, train_list)
    file_utils.write_list_data(val, val_list)
    print("{},trainval:{},train:{},val:{}".format(file_dir,len(file_list), len(train_list), len(val_list)))


if __name__ == "__main__":
    class_name = "/home/dm/nasdata/dataset/csdn/hand/HaGRID/class_name.txt"
    class_name = file_utils.read_data(class_name,split=None)
    for name in class_name:
        annotations_dir = "/home/dm/nasdata/dataset/csdn/hand/HaGRID/trainval/{}/JPEGImages".format(name)
        save_file_list(annotations_dir, prefix="", postfix=image_utils.IMG_POSTFIX, only_id=False, shuffle=False, nums=None)
